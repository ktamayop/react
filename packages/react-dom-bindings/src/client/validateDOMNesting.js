...
function findInvalidAncestorForTag(tag, ancestorInfo) {
  const parentTags = getPossibleInheritorsForTag(tag);
  for (let i = 0; i < parentTags.length; i++) {
    const parentTag = parentTags[i];
    const ancestor = getAncestorInfoForTag(parentTag, ancestorInfo);
    if (ancestor) {
      return ancestor;
    }
  }
...
function getPossibleInheritorsForTag(tag) {
  const inheritors = [];
  if (tag in PTagAncestor) {
    inheritors.push(tag);
  }
  if (tag === 'form') {
    inheritors.push(tag, ...PTagAncestor);
  }
  if (tag === 'li') {
    inheritors.push('li');
  }
  ...
  if (tag in InScopeTags) {
    inheritors.push(InScopeTags[tag]);
  }
  return inheritors;
}

function getAncestorInfoForTag(tag, ancestorInfo) {
  const ancestorValue = PTagAncestor[tag] || tag;
  return ancestorInfo[ancestorValue] || null;
}
